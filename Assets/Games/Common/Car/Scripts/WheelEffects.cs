...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class WheelEffects : MonoBehaviour
    {
        ...
        {
            skidParticles.transform.position = transform.position - transform.TransformVector(transform.up)*m_WheelCollider.radius;
        ...
        public IEnumerator StartSkidTrail()
        {
            ...
            m_SkidTrail.SetParent(transform, false);
            var trail = m_SkidTrail.GetComponent<TrailRenderer>();
            if (trail != null) {
                var size = transform.TransformVector(Vector3.forward).magnitude;
                trail.widthMultiplier *= size;
                trail.minVertexDistance *= size;
            }
